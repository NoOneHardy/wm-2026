...
export const emailVerificationLinkSent = createAction('[User] Email verification link sent', props<{
  isSent: boolean
}>())

// Password reset
export const requestPasswordResetLink = createAction('[User] Request password reset link', props<{ email: string }>())
export const passwordResetLinkRequested = createAction('[User] Password reset link requested', props<{
  isSent: boolean
}>())
